...
prompt_deploy_mode() {
  ...
  while true; do
    ...
    mode=${mode:-1}
    case "$mode" in
      1|desktop|Desktop) MODE="desktop"; break ;;
      2|router|Router)   MODE="router";  break ;;
      *) echo "Invalid choice — enter 1 (desktop) or 2 (router)." ;;
    esac
  done
}
...
build_cake_opts() {
  if [ "$MODE" = "desktop" ]; then
    EGRESS_OPTS=(oceanic diffserv4 conservative flows split-gso nat nowash memlimit 32mb)
    INGRESS_OPTS=(ingress oceanic diffserv4 conservative flows split-gso nat nowash memlimit 32mb)
  else
...
save_persistence_config() {
  local ifc=$1 up=$2 down=$3 mode=$4
  local conf_dir="/etc/cake-sqm"
  mkdir -p "$conf_dir"
  cat > "$conf_dir/$ifc.conf" <<PERSIST
# CAKE SQM config for $ifc — auto-generated by cake-sqm-setup.sh
IFACE="$ifc"
UPLOAD_BW="$up"
DOWNLOAD_BW="$down"
MODE="$mode"
PERSIST
  chmod 644 "$conf_dir/$ifc.conf"
  _green "Saved configuration to $conf_dir/$ifc.conf"
}
...
[Service]
Type=oneshot
ExecStart="SCRIPT_PATH" --restore %I
RemainAfterExit=yes
...
remove_persistence_config() {
  local ifc=$1
  local conf="/etc/cake-sqm/$ifc.conf"

  if systemctl is-enabled "cake-sqm-restore@$ifc.service" >/dev/null 2>&1; then
    systemctl disable "cake-sqm-restore@$ifc.service"
    echo "Disabled systemd service: cake-sqm-restore@$ifc.service"
  fi

  if [ -f "$conf" ]; then
    rm -f "$conf"
    echo "Removed saved config: $conf"
  fi

  rmdir /etc/cake-sqm 2>/dev/null || true
  _green "Persistence fully removed for $ifc"
}
...
restore_config() {
  ...
  source "$conf"
  build_cake_opts
  echo "Restoring CAKE for $ifc (mode: $MODE)..."
  if [ "$UPLOAD_BW" != "SKIP" ]; then
    apply_cake_egress "$ifc" "$UPLOAD_BW" "${EGRESS_OPTS[@]}"
  fi
  if [ "$DOWNLOAD_BW" != "SKIP" ]; then
    create_ifb "$ifc"
    IFBDEV="ifb-$ifc"
    setup_ingress_redirect "$ifc" "$IFBDEV"
    apply_cake_ingress "$IFBDEV" "$DOWNLOAD_BW" "${INGRESS_OPTS[@]}"
  fi
  _green "CAKE restored for $ifc"
}
...
main() {
  ...
  if ask_yes_no "Make this CAKE configuration persistent across reboots?"; then
...
if [ "${1-}" = "--remove" ]; then
  ...
  remove_cake "$2"
  if [ -f "/etc/cake-sqm/$2.conf" ]; then
    echo
    _yellow "WARNING: Persistent boot-time CAKE config exists for $2."
    _yellow "CAKE will be restored on next reboot."
    echo "Run '$PROG_NAME --unpersist $2' to fully remove persistence."
  fi
  exit 0
fi

# Support non-interactive restore from saved config: --restore IFACE
if [ "${1-}" = "--restore" ]; then
  check_root_or_sudo "$@"
  if [ -z "${2-}" ]; then echo "Usage: $PROG_NAME --restore <iface>"; exit 1; fi
  restore_config "$2"; exit 0
fi

# Support removing persistent boot-time restore: --unpersist IFACE
if [ "${1-}" = "--unpersist" ]; then
  check_root_or_sudo "$@"
  if [ -z "${2-}" ]; then echo "Usage: $PROG_NAME --unpersist <iface>"; exit 1; fi
  remove_persistence_config "$2"; exit 0
fi
